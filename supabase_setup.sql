...
CREATE TRIGGER trigger_users_updated_at BEFORE UPDATE ON users FOR EACH ROW EXECUTE FUNCTION handle_updated_at();
CREATE TRIGGER trigger_products_updated_at BEFORE UPDATE ON products FOR EACH ROW EXECUTE FUNCTION handle_updated_at();
CREATE TRIGGER trigger_receipt_settings_updated_at BEFORE UPDATE ON receipt_settings FOR EACH ROW EXECUTE FUNCTION handle_updated_at();
CREATE TRIGGER trigger_blog_posts_updated_at BEFORE UPDATE ON blog_posts FOR EACH ROW EXECUTE FUNCTION handle_updated_at();
CREATE TRIGGER trigger_site_settings_updated_at BEFORE UPDATE ON site_settings FOR EACH ROW EXECUTE FUNCTION handle_updated_at();
CREATE TRIGGER trigger_pages_updated_at BEFORE UPDATE ON pages FOR EACH ROW EXECUTE FUNCTION handle_updated_at();
...
-- 7. RPC for Atomic Order Placement
...
-- 8. AUTH TRIGGER: Automatically create a profile in the 'users' table when a new user signs up
...
BEGIN
  ...
  VALUES (new.id, COALESCE(new.raw_user_meta_data->>'username', new.email), v_role);
  RETURN new;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
